...
async def test_session_cookie_not_forwarded_to_upstream() -> None:
    ...
    async with AsyncClient(
        ...
        await _login(client)
        response = await client.get("/check", follow_redirects=False)
...
async def test_logout_clears_session() -> None:
    ...
    async with AsyncClient(
        ...
        after_resp = await client.get("/workers", follow_redirects=False)
...
async def test_invalid_credentials_do_not_grant_access() -> None:
    ...
    async with AsyncClient(
        ...
        response = await client.get("/workers", follow_redirects=False)
