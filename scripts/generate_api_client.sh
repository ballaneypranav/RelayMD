...
RELAYMD_CONFIG=/dev/null \
INFISICAL_TOKEN=schema-export \
PYTHONPATH="$ROOT_DIR/src:$ROOT_DIR/packages/relaymd-core/src${PYTHONPATH:+:$PYTHONPATH}" \
  "$VENV_PYTHON" -c "import json; from pathlib import Path; from relaymd.orchestrator.config import OrchestratorSettings; from relaymd.orchestrator.main import create_app; settings = OrchestratorSettings(api_token='schema-export-token', axiom_token='schema-export-token', infisical_token='schema-export'); app = create_app(settings=settings, start_background_tasks=False); Path('$SCHEMA_PATH').write_text(json.dumps(app.openapi(), indent=2), encoding='utf-8')"
